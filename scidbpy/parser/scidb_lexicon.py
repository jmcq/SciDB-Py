...
DEF array aggregate(array, numeric)
DEF void store(array, array)
DEF void remove(array)
DEF void scan(array)
DEF void show(array|string, [string])
DEF array build(schema|array, numeric)
DEF array apply(array, [attr, numeric]+)
DEF array project(array, [attr]+)

DEF array multiply(array, array)
"""


SCIDB_COERSIONS = {
    'numeric': ['int', 'float', 'attr', 'dim', 'identifier'],
    'array': ['identifier'],
    'attr': ['identifier'],
    'dim': ['identifier'],
    'float': ['attr', 'dim', 'identifier'],
    'int': ['attr', 'dim', 'identifier']
}
...
class FunctionObj(object):
    ...
    def matches_call(self, input_arglist):
        if len(input_arglist) < len(self.args):
            return False

        for arg_in, args_expected in zip(input_arglist, self.args):
            if arg_in in args_expected:
                continue
            elif arg_in in itertools.chain(*(SCIDB_COERSIONS.get(arg, [])
                                             for arg in args_expected)):
                continue
            else:
                return False

        extra_args = input_arglist[len(self.args):]
        if len(extra_args) > 0:
            raise NotImplementedError("optional arg checking")
        return True
